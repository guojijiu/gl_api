...
func (suite *APIIntegrationTestSuite) SetupSuite() {
	...
	suite.storageManager = Storage.NewStorageManager(storageConfig)

	// 初始化数据库
	Database.InitDBWithLogger(suite.storageManager)
...
func (suite *APIIntegrationTestSuite) initAPIEngine() {
	...
	suite.APIEngine.Use(corsMiddleware.Handle())

	// 创建模拟的LogManagerService用于RequestLogMiddleware
	...
	suite.APIEngine.Use(requestLogMiddleware.RequestLog())

	// 注册路由 - 使用RegisterRoutes函数
	Routes.RegisterRoutes(suite.APIEngine, suite.storageManager, logManager)
}
...
func (suite *APIIntegrationTestSuite) createTestUsers() {
	...
	Database.DB.Create(suite.TestUser)

	// 创建管理员测试用户
	suite.TestAdminUser = &Models.User{
		Username: "adminuser",
	...
	Database.DB.Create(suite.TestAdminUser)
...
func (suite *APIIntegrationTestSuite) TestAPI_UserRegistration() {
	...
	err = Database.DB.Where("username = ? AND email = ?", "newuser", "newuser@example.com").First(&user).Error
	require.NoError(t, err)
}
...
func (suite *APIIntegrationTestSuite) TestAPI_UpdateUserProfile() {
	...
	// 验证数据库记录已更新
	var user Models.User
	err = Database.DB.Where("id = ?", suite.TestUser.ID).First(&user).Error
	require.NoError(t, err)
...
func (suite *APIIntegrationTestSuite) TestAPI_CreateTag() {
	...
	// 验证数据库记录
	var tag Models.Tag
	err = Database.DB.Where("name = ?", "testtag").First(&tag).Error
	require.NoError(t, err)
}
...
func (suite *APIIntegrationTestSuite) TestAPI_GetTags() {
	...
	// 创建测试标签
	tag1 := &Models.Tag{Name: "listtag1", Description: "List tag 1"}
	tag2 := &Models.Tag{Name: "listtag2", Description: "List tag 2"}
...
func (suite *APIIntegrationTestSuite) TestAPI_UpdateTag() {
	...
	// 创建测试标签
	tag := &Models.Tag{Name: "updatetag", Description: "Original description"}
	...
	err = Database.DB.Where("id = ?", tag.ID).First(&updatedTag).Error
	require.NoError(t, err)
	assert.Equal(t, "updatedtag", updatedTag.Name)
...
func (suite *APIIntegrationTestSuite) TestAPI_DeleteTag() {
	...
	// 创建测试标签
	tag := &Models.Tag{Name: "deletetag", Description: "Tag to delete"}
	Database.DB.Create(tag)
	...
	err = Database.DB.Where("id = ?", tag.ID).First(&deletedTag).Error
...
func (suite *APIIntegrationTestSuite) TestAPI_GetApiKeys() {
	...
	// 创建测试API密钥
	apiKey1 := &Models.ApiKey{UserID: suite.TestUser.ID, Name: "listapikey1", KeyHash: "test-key-1-hash", Prefix: "test1", Status: 1}
	apiKey2 := &Models.ApiKey{UserID: suite.TestUser.ID, Name: "listapikey2", KeyHash: "test-key-2-hash", Prefix: "test2", Status: 1}
	Database.DB.Create(apiKey1)
	Database.DB.Create(apiKey2)
