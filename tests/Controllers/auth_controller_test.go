...
func (ats *AuthControllerTestSuite) TestLogin() {
	// 先创建测试用户
	userInfo, err := ats.CreateTestUserWithToken("logintest", "login@example.com", "password123", "user")
	ats.Require().NoError(err)
...
func (ats *AuthControllerTestSuite) TestPasswordReset() {
	// 创建测试用户
	userInfo, err := ats.CreateTestUserWithToken("resetuser", "reset@example.com", "password123", "user")
	ats.Require().NoError(err)
